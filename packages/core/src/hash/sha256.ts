// Pure, synchronous SHA-256 (hex output). No dependencies.
//
// Core compiles with `types: []` and imports nothing (ADR-0003), so neither
// `node:crypto` nor async Web Crypto is available. Atom identity (ADR-0002) is
// load-bearing and must be sync, so SHA-256 is implemented here and tested
...
function toUtf8Bytes(input: string): Uint8Array {
  // Manual UTF-8 encoding keeps this dependency-free (no TextEncoder typings under `types: []`).
  const out: number[] = [];
...
export function sha256Hex(input: string): string {
  ...
  // 64-bit length; bitLen fits in 32 bits for any realistic input, high word stays 0.

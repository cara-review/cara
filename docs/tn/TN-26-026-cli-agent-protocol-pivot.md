...
| 2 | **Built-in agent dropped as a driven port.** It reforms as an outer LLM-wrapper porcelain (`cara review`) that drives the same plumbing verbs. `AgentPort` inverts: grouping arrives *inbound* (untrusted, via `repairGrouping`), never fetched. |
...
| 8 | **`~/.cara/config.toml`. No silent fallbacks.** Behavior configured, never inferred. Keys referenced by env-var name only. |
| 9 | **Chat pane dropped. The comment is the only interface.** Freeform in, structured out. |
| 10 | **cara owns the methodology.** `atoms` emits system methodology + project `CARA.md` + personal `~/.cara/CARA.md`, merged, version-locked to the `present` schema. |
...
| 13 | **Onboarding = self-narrating protocol + `cara instructions` + a documented one-liner.** No shipped skill. |

## The verb contract

All four are **agent-invoked**; cara never initiates toward the agent (no callback channel — that's what makes it portable). Verbs differ in payload direction only.
...
- **Self-narrating protocol:** every JSON response carries a `next` hint ("now run: …"). A cold agent that runs any one verb is pulled through the whole loop. `cara instructions` emits the canonical loop + verb reference, generated from the same source — version-locked, no doc drift.
...
Subsumes `AppConfig.groupingModel`; becomes the `ConfigPort` source. Instruction files renamed `cara.md` → `CARA.md` (project root) and `~/.cara/CARA.md` (personal) in the implementing change.

## Agent onboarding — three tiers

1. **Inline (zero setup):** user says "review this with cara"; agent runs `cara instructions`; the self-narrating protocol does the rest.
2. **One-liner (recommended):** paste into CLAUDE.md / AGENTS.md / a rule: *"To review changes with the user, run `cara instructions` and follow it."*
...
`cara gate` / risk tiers · RCR artifact + verifier · standing/cross-context store · fleet mode · MCP adapter · lens fan-out. The pivot must not preclude them; the event-log + tier substrate is exactly what they build on.
...
3. Porcelain: `cara review` wrapper (grouping call, answer calls); config.toml; lazy keys.

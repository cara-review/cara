...
- `--require <role>{=|>=}<percent>%`, comma-separated (e.g. `security=100%,human>=50%`). Roles:
  `addressed`, `accounted`, the tiers `human`/`agent`, or a reviewer label. Exact-ratio comparison;
  empty diff vacuously met; no `--require` is a readout that never fails.
...
- **Scrutiny predicates** (anti-rubber-stamp): the sweep footprint is recorded but not yet gateable.

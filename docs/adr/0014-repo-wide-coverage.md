...
- An **empty net range is `indeterminate`, never a vacuous pass** (a distinct, non-zero, non-failure exit code). A zero-atom denominator repo-wide almost always means a misconfigured baseline/target, not "nothing to review" — so unlike the per-diff vacuous-met rule (ADR-0013 §4, unchanged), `--repo` refuses to report a silent green over an empty set. (Ratified as shipped: the deliberate divergence from ADR-0013 §4 stands.)

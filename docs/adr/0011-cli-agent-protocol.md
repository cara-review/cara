...
## Amendment (2026-06-12): discovery banner on bare invocation

A cold agent that runs `cara` must reach the protocol without prior knowledge. Bare `cara`, `cara --help`/`-h`, and `cara help [<verb>]` now print an orientation **banner** that signposts `cara instructions` (and, with a verb topic, a terse per-verb synopsis). The bare invocation **no longer routes to the `review` porcelain** — the porcelain is reached only by explicit `cara review …` or a leading `<base>..<head>` range, so discovery never loads the LLM or needs a key.

`help` is a **discovery surface, not a protocol verb**: plain text, no JSON envelope, no `next` hint, no accounting. It signposts the self-narrating loop; it does not participate in it. The banner and per-verb usage are terse synopses that reuse the shared spec/payload grammar — the methodology lives only in `instructions`, never restated.

...
clear-diff is built as a **local web app**: the `clear-diff` CLI boots a localhost HTTP/WS server and opens the UI in an `--app`-mode browser window, architected so wrapping it in **Electron** later is a thin, additive change. The in-host MCP option is rejected outright; a native desktop shell is deferred, not chosen.
...
- **Standalone desktop (Tauri)** — *rejected.* Tauri's Rust backend mismatches clear-diff's Node logic, forcing a rewrite or a Node sidecar and negating its lightness advantage.

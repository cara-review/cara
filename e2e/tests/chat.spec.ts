...
test("asking a question shows the question and the agent's answer", async ({ page }) => {
  ...
  await expect(page.locator(".chat__message--user")).toHaveText("is this backwards compatible?");
  await expect(page.locator(".chat__message--agent")).toContainText("this Chapter has");
